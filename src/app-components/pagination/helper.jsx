...
export const determinePagesToShow = (pageCount, currentPage, setPage) => {
  ...
  if (pageCount > 6) {
    if (currentPage === 0) {
      return [
        createPage(currentPage, setPage, 1),
        createPage(currentPage, setPage, 2),
        <Ellipsis key='ellipsis-0' />,
      ];
    }
    if (currentPage === pageCount - 1) {
      return [
        <Ellipsis key='ellipsis-1' />,
        createPage(currentPage, setPage, currentPage - 2),
        createPage(currentPage, setPage, currentPage - 1),
      ];
    }
    ...
    return new Array(pageCount)
      .fill(null)
      .slice(1, pageCount - 1)
      .map((_page, i) => createPage(currentPage, setPage, i + 1));
  }
