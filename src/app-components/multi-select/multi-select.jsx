    ...
    if (isFilterable) {
      if (previuosInputValue !== inputValue) {
        const newSet = options.filter((elem) => {
          ...
          return (
            String(val).toLowerCase().indexOf(inputValue.toLowerCase()) !== -1
          );
        });
      ...
      {filteredOptions.length ? (
        <>
          ...
          {filteredOptions.map((option, i) => {
            const optionIsSelected = currentSelections.find(
              (elem) => elem === option.value
            );
            const handleClick = () =>
              setCurrentSelections(
                reduceSelections(option.value, currentSelections)
              );
