  ...
  doUploadAllFiles:
    ...
    ({ dispatch, apiPost, store }) => {
      ...
      const payload = {
        ...
        ...(siteFile && {
          siteUpload: { uploadFilename: files.siteFile.name, items: siteFile },
        }),
        ...(searchEffortFile && {
          searchUpload: {
            uploadFilename: files.searchEffortFile.name,
            items: searchEffortFile,
          },
        }),
        ...(telemetryFishFile && {
          telemetryUpload: {
            uploadFilename: files.telemetryFishFile.name,
            items: telemetryFishFile,
          },
        }),
        ...(missouriRiverFile && {
          moriverUpload: {
            uploadFilename: files.missouriRiverFile.name,
            items: missouriRiverFile,
          },
        }),
        ...(fishFile && {
          fishUpload: { uploadFilename: files.fishFile.name, items: fishFile },
        }),
        ...(supplementalFile && {
          supplementalUpload: {
            uploadFilename: files.supplementalFile.name,
            items: supplementalFile,
          },
        }),
        ...(proceduresFile && {
          procedureUpload: {
            uploadFilename: files.proceduresFile.name,
            items: proceduresFile,
          },
        }),
      };
